...
protector.node = "server_player:protect"
protector.item = "server_player:stick"
...
minetest.register_node(protector.node, {
	...
	on_punch = function(pos, node, puncher)
		...
		for _, o in pairs(objs) do
			if (not o:is_player()) and o:get_luaentity().name == "server_player:display" then
				o:remove()
				removed = true
			end
		end
		if not removed then -- nothing was removed: there wasn't the entity
			minetest.env:add_entity(pos, "server_player:display")
		end
	end,
})
...
minetest.register_node("server_player:display_node", {
